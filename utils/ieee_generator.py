...
def generate_latex_formula_image(latex_code: str) -> str | None:
    """
    Convert LaTeX formula to image using matplotlib.
    Handles both LaTeX syntax and plain text formulas.
    ...
    try:
        # Clean the input - remove existing $ symbols if present
        ...
        # Generate unique filename based on formula content
        ...
        ax.text(0.5, 0.5, f"${clean_code}$", fontsize=16, ha='center', va='center', 
                transform=ax.transAxes)
        ax.axis('off')
        
        # Save with high DPI and tight bounding box
        plt.savefig(filepath, format='png', bbox_inches='tight', 
                   transparent=True, dpi=300, pad_inches=0.1)
        plt.close(fig)
        return filepath
        
    except Exception as e:
        logger.error(f"Formula rendering failed for '{latex_code}': {e}")
        # Try fallback with plain text rendering
        try:
            # Create images directory if it doesn't exist
            images_dir = os.path.join(os.path.dirname(os.path.dirname(__file__)), 'images')
            os.makedirs(images_dir, exist_ok=True)
            ...
            # Generate unique filename based on formula content
            formula_hash = hashlib.md5(latex_code.encode()).hexdigest()[:8]
            filename = f"formula_fallback_{formula_hash}.png"
            filepath = os.path.join(images_dir, filename)
            
            fig, ax = plt.subplots(figsize=(6, 1))
            ax.text(0.5, 0.5, latex_code, fontsize=14, ha='center', va='center',
            ...
            logger.error(f"Fallback formula rendering also failed: {e2}")
            return None
    ...
    try:
        ...
        for i, sec_data in enumerate(data['sections'], 1):
            ...
            for table in sec_data.get("tables", []):
                ...
                # Add spacing before caption
                caption_para = doc.add_paragraph()
                caption_para.add_run(f"Table {tbl_ct}: Data Table")
